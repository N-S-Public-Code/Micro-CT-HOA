...
def linear_blending_diff_width(img1, img2, alpha):
    # Determine the overlapping region
    rows1, cols1 = img1.shape
    rows2, cols2 = img2.shape
    min_cols = min(cols1, cols2)
    
    # Create a result image with width equal to the maximum width of the two images
    result = np.zeros((rows1, max(cols1, cols2)), dtype=np.uint16)
    
    # Copy non-overlapping regions from img1 and img2
    result[:, :cols1] = img1
    result[:, :cols2] = np.maximum(result[:, :cols2], img2)
    
    # Blend the overlapping region
    result[:, :min_cols] = cv2.addWeighted(img1[:, :min_cols], alpha, img2[:, :min_cols], 1 - alpha, 0)
    
    return result

def calculate_dynamic_alpha(img1, img2):
    avg_intensity1 = np.mean(img1)
    avg_intensity2 = np.mean(img2)
    total_intensity = avg_intensity1 + avg_intensity2
    alpha = avg_intensity1 / total_intensity
    #if total_intensity != 0 else 0.5
    return alpha
...
def corp_to_bounds(image):
    # Corpping the image
    # Threshold the image to create a binary image
    _, binary_panorama = cv2.threshold(image, 1, 65535, cv2.THRESH_BINARY)
    ...
    cropped = image[y:y+h, x:x+w].astype(np.uint16)
    return cropped 

def stitch_images(image1, image2):
    shift_x, shift_y = phase_correlation(image1, image2)
    translated_image2 = translate_image(image2, -1*shift_x, shift_y)
    image1 =corp_to_bounds(image1)
    translated_image2 =corp_to_bounds(translated_image2)
    
    a = calculate_dynamic_alpha(image1, translated_image2)
    blended = linear_blending_diff_width(image1, translated_image2, a)
...
file1 = r'F:\Natanel_DO_NOT_DELETE\ESRF_Binned_SuperCleanRecon\73\HOA\stack1.tif'
# Radiographs for 180-360 deg rotation (rigth side)
file2 = r'F:\Natanel_DO_NOT_DELETE\ESRF_Binned_SuperCleanRecon\73\HOA\stack2_flipped.tif'
saving_path = r"F:\Natanel_DO_NOT_DELETE\ESRF_Binned_SuperCleanRecon\73\HOA\stitched_HM_PhaseCorr"
log_path = saving_path
stack1 = read_tiff_stack(file1)

#Rectangular Region of Interest
ROI = (0, 0, 2050, 960) # Define your ROI (x, y, width, height)
